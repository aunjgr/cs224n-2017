    ...
    for sentence, labels in data:
        for i in range(len(sentence)):
            ...
            for j in range(i - window_size, i + window_size + 1):
                feature.extend(start if j < 0 else end if j >= len(sentence) else sentence[j])
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("W", shape=(Config.n_window_features * Config.embed_size, Config.hidden_size), initializer=initializer)
        b1 = tf.get_variable("b1", (Config.hidden_size,))
        U = tf.get_variable("U", (Config.hidden_size, Config.n_classes), initializer=initializer)
    ...
    def preprocess_sequence_data(self, examples):
        return make_windowed_data(examples, start=self.helper.START, end=self.helper.END, window_size=Config.window_size)
